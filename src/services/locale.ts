...
export const editLocaleById = (locale: LocaleState): ThunkAction<Promise<any>, {}, {}, AnyAction> => {
    return async function (dispatch: any) {
        let editLocale = null;
        try {
            const response = await httpClient.put(`http://localhost:3000/api/v1/locales/${locale.id}`, locale);
            if (response !== null && typeof response.data !== 'undefined') {
...
export const addLocale = (locale: LocaleState): ThunkAction<Promise<any>, {}, {}, AnyAction> => {
    return async function (dispatch: any) {
        let newLocale = null;
        try {
            const response = await httpClient.post(`http://localhost:3000/api/v1/locales`, locale);
            if (response !== null && typeof response.data !== 'undefined') {
